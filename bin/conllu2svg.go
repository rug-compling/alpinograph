...
func conllu2svg(conllu, idlist, edgelist string) (string, bool) {
	...
	if edgelist != "" {
		for _, e := range strings.Split(edgelist, ",") {
			a := strings.SplitN(e, "_", 4)
			if len(a) != 4 || (a[0] != "u" && a[0] != "e") {
				continue
			}
			for i := 1; i < 3; i++ {
				b := strings.SplitN(a[i], "!", 2)
				if len(b) == 2 {
					a[i] = b[1]
				} else if a[i] == "-1" {
					a[i] = "0"
			...
			key := fmt.Sprintf("%s:%s:%s", a[2], a[1], a[3])
			if a[0] == "u" {
				ud1[key] = true
			} else {
				ud2[key] = true
	...
	// x(fmt.Errorf("ud1: %v, ud2: %v\n", ud1, ud2))
	// return "", false

	var buf bytes.Buffer
	fp := &buf

	bolds := make(map[string]bool)
	if idlist != "" {
		for _, id := range strings.Split(idlist, ",") {
			bolds[id2end[id]] = true
		}
	}
...
function mark(id, i) {
  ...
  if (!holding) {
    // $('svg#' + id + ' .l' + cl + i).css({'fill':'green','font-weight':'bold'});
    // $('svg#' + id + ' .e' + cl + i).css({'stroke':'green','stroke-width':3});
    $('svg#' + id + ' .l' + cl + i).css({'font-weight':'bold'});
    $('svg#' + id + ' .e' + cl + i).css({'stroke-width':3});
  }
}
function unmark(id, i) {
  ...
  if (!holding) {
    ...
    $('svg#' + id + ' .l' + cl + i).css({'font-weight':'normal'});
    $('svg#' + id + ' .e' + cl + i).css({'stroke-width':1});
  }
}
function mrk(id, i, j) {
  if (!holding) {
    // $('svg#' + id + ' .p' + i + 'p' + j).css({'fill':'green','font-weight':'bold'});
    // $('svg#' + id + ' .q' + i + 'q' + j).css({'stroke':'green','stroke-width':3});
    // $('svg#' + id + ' .q' + i).css({'stroke':'green','stroke-width':3});
    // $('svg#' + id + ' .q' + j).css({'stroke':'green','stroke-width':3});
    $('svg#' + id + ' .p' + i + 'p' + j).css({'font-weight':'bold'});
    $('svg#' + id + ' .q' + i + 'q' + j).css({'stroke-width':3});
    $('svg#' + id + ' .q' + i).css({'stroke-width':3});
    $('svg#' + id + ' .q' + j).css({'stroke-width':3});
  }
}
function unmrk(id, i, j) {
  if (!holding) {
    ...
    $('svg#' + id + ' .p' + i + 'p' + j).css({'font-weight':'normal'});
    $('svg#' + id + ' .q' + i + 'q' + j).css({'stroke-width':1});
    $('svg#' + id + ' .q' + i).css({'stroke-width':1});
    $('svg#' + id + ' .q' + j).css({'stroke-width':1});
  }
}
</script>
`)
	fmt.Fprintf(fp, `<div class="svg">
...
`, svgID, width, height)
	...
	fmt.Fprintln(fp, "<g fill=\"#c0c0ff\" stroke=\"black\" stroke-width=\"1\">")
	for i, item := range items {
		enh := ""
		color := ""
		if item.enhanced {
		...
		if bolds[item.here] {
			// color += `fill="#ffffff" stroke="#0000ff" stroke-width="3" `
			color += `fill="#ffffff" stroke="#0000ff" `
		}
...
%s
	...
	check1 := ""
	check2 := " checked"
	if !hasEnhanced || (len(ud2) == 0 && len(ud1) > 0) {
